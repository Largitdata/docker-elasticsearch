...
if [ "$NODE_MASTER" == "true" ]; then
    echo "Mode:"
    echo "   => MASTER"
    echo "   => Waiting 10 seconds for discovery service ready"
    sleep 10
    MASTER_IP_LIST=`dig +short +search $DISCOVERY_SERVICE|paste -s -d","`
    echo "   => Waiting 10 seconds for discovery service ready"
    echo "   => IP list <${MASTER_IP_LIST}>"
    if [ ! "$MASTER_IP_LIST" = "" ];
    then
        MASTER_ARGS="-Ecluster.initial_master_nodes=${MASTER_IP_LIST}"
    fi
fi
...
if [[ $(whoami) == "root" ]]; then
    ...
    exec su-exec elasticsearch $BASE/bin/elasticsearch $ES_EXTRA_ARGS $MASTER_ARGS
else
    ...
    "${BASE}"/bin/elasticsearch ${ES_EXTRA_ARGS} $MASTER_ARGS
fi
